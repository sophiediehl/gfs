...
def processor(year, month, day): # i.e. '2014', '9', '5'
  ...
  for k in range(8):
    if k<4: fn = 'gfs_rawdata/gfs_4_'+timestr+'_0000_00'+str(k*3)+'.grb2'
    else: fn = 'gfs_rawdata/gfs_4_'+timestr+'_0000_0'+str(k*3)+'.grb2'
    gr = pygrib.open(fn)
  ...
  with open('gfs_hdfs_in/v'+timestr+'.csv', 'wb') as f:
    writer = csv.writer(f)
    writer.writerows(results)
  return 'gfs_hdfs_in/v'+timestr+'.csv'
